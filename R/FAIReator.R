...
#' This function is to generate FAIR eDNA data templates, based on the sample
#' types, assay type, and requirement levels of your choice.
#'
#' @param req_lev Requirement level(s) of each fields to include in the data
#'   template. Select one or more from "M", "HR", "R", and "O" (Mandatory,
#'   Highly recommended, Recommended, and Optional, respectively). Default is
#'   c("M", "HR", "R", "O")
#' @param sample_type A (list of) Sample type(s). Select one or more from
#'   "Water", "Soil", "Sediment", "Air", "HostAssociated",
#'   "MicrobialMatBiofilm", and "SymbiontAssociated", or "other". "other" will
#'   include all the sample-type-specific fields in the output.
#' @param assay_type An approach applied to detect taxon/taxa. Select one
#'   "targeted" (e.g., q/d PCR based detection) or "metabarcoding" (e.g.,
#'   metabarcoding)
#' @param project_id A brief, concise project identifier with no spaces or
#'   special characters. This ID will be used in data file names as
#'   'project_id'.
#' @param assay_name A brief, concise assay name(s) with no spaces or special
#'   characters. This will be used in data file names as 'assay_name'.
#' @param projectMetadata_user (optional) A user-defined field or list of fields
#'   that are not listed in the FAIR eDNA metadata checklist. These fields will
#'   be appended to the end of the projectMetadata.
#' @param sampleMetadata_user (optional) A user-defined field or list of fields
#'   that are not listed in the FAIR eDNA metadata checklist. These fields will
#'   be appended to the end of the sampleMetadata.
#' @param return_wb_object (optional) return a wb xlsx object for debugging.
#'
#' @return The expected outputs are:
#' A new folder called "template_<project_id>" (e.g., template_gbr2022) in your
#' working directory. An Excel file called "<project_id>.xlsx" (e.g.,
#' gbr2022.xlsx) in the new template folder, containing multiple worksheets in
#' the file, including README, projectMetadata, sampleMetadata and other
#' assay_type specific tables (e.g., OTU table if assay_type = "metabarcoding")
#' Optional returns a 'wb' xlsx object for testing.
#' @export
#' @importFrom magrittr `%>%`
#' @importFrom openxlsx dataValidation readWorkbook createWorkbook addWorksheet writeData addStyle createStyle writeComment createComment saveWorkbook
...
FAIReator <- function(req_lev = c("M", "HR", "R", "O"),
                      ...
                      return_wb_object = FALSE) {
  # set input and output ----------------------------------------------------
  system_file_path <- system.file("extdata",
    package = "fairr"
  )
  FAIRe_checklist_ver <- "v1.0.2"
  checklist_path <- paste0(
    system_file_path,
    "/templates/FAIRe_checklist_",
    FAIRe_checklist_ver,
    ".xlsx"
  )
  ...
  if (!is.null(projectMetadata_user)) {
    readme2 <- c(
      ...
      paste(
        ...
        paste(projectMetadata_user, collapse = " | ")
      )
    )
  }
  if (!is.null(sampleMetadata_user)) {
    readme2 <- c(
      ...
      paste(
        "sampleMetadata_user =",
        paste(sampleMetadata_user, collapse = " | ")
      )
    )
  }
  ...
  if (assay_type == "metabarcoding") {
    readme4 <- c(
      readme4, # MT: add exp meta for multi taxon.
  ...
  for (i in req_col_df$requirement_level_code) {
    addStyle(
      ...
      sheet = "README",
      style = createStyle(
        fgFill = req_col_df$col[which(req_col_df$requirement_level_code == i)]
      ),
      rows = which(
        readme.df[, 1] == req_col_df$requirement_level[
          which(req_col_df$requirement_level_code == i)
        ]
      ),
      cols = 1,
      gridExpand = T,
  ...
  if (!is.null(projectMetadata_user)) {
    temp <- data.frame(matrix(
      nrow = length(projectMetadata_user),
      ncol = ncol(sheet_df)
    ))
  ...
  sheet_df[
    which(sheet_df$term_name == "project_id"),
    "project_level"
  ] <- project_id
  sheet_df[
    which(sheet_df$term_name == "assay_type"),
    "project_level"
  ] <- assay_type
  sheet_df[
    which(sheet_df$term_name == "checkls_ver"),
    "project_level"
  ] <- checklist_path
  if (length(assay_name) == 1) {
    sheet_df[
      which(sheet_df$term_name == "assay_name"), "project_level"
    ] <- assay_name
  } else if (length(assay_name) > 0) {
    for (i in 1:length(assay_name)) {
      sheet_df[, paste0("assay", i)] <- NA
      sheet_df[
        which(sheet_df$term_name == "assay_name"),
        paste0("assay", i)
      ] <- assay_name[i]
    }
  }
  ...
  samp_type_row2keep <- if (any(sample_type %>% tolower() == "other")) {
    ...
    filtered_input <- input %>% dplyr::filter(
      is.na(input$sample_type_specificity) |
        input$sample_type_specificity == "ALL" |
        grepl(paste(sample_type, collapse = "|"),
          input$sample_type_specificity,
          ignore.case = TRUE
        )
    )
    ...
    sheet_df <- sheet_df[
      ,
      intersect(filtered_input$term_name, colnames(sheet_df))
    ]
  }
  ...
  if (assay_type == "targeted" & length(assay_name) > 1) {
    ...
    new_cols_data <- as.data.frame(
      replicate(
        length(new_columns),
        sheet_df[, "detected_notDetected"]
      )
    )
    ...
    sheet_df <- cbind(
      sheet_df[, 1:(original_col_index - 1)], # Columns before the original
      new_cols_data,
      sheet_df[, original_col_index:ncol(sheet_df)]
    ) # Columns after the original
  }
  ...
  if (!is.null(sampleMetadata_user)) {
    temp <- data.frame(
      matrix(
        nrow = nrow(sheet_df),
        ncol = length(sampleMetadata_user)
      )
    )
  ...
  if (any(colnames(sheet_df) == "assay_name")) {
    sheet_df[
      nrow(sheet_df) + 1, "assay_name"
    ] <- paste(assay_name, collapse = " | ")
  }
  ...
  # comment should include requirement level (and requirement level condition if
  # any), description, example, field type, and field type option (for cont.
  # vocab) or field format (field format i.e. date)
  # text styles
  c1 <- createStyle(
    fontColour = "black"
  )
    ...
    if (sheet_name == "projectMetadata") {
      ...
      for (i in req_lev) {
        addStyle(
          ...
          sheet = sheet_name,
          style = createStyle(
            fgFill = req_col_df$col[
              which(req_col_df$requirement_level_code == i)
            ]
          ),
          rows = which(sheet_df$requirement_level_code == i) + 1,
          cols = which(
            colnames(sheet_df) == "requirement_level_code"
          ),
          gridExpand = T,
          stack = T
        )
      ...
      addStyle(wb,
        sheet = sheet_name,
        style = c_bold,
        rows = 1:(nrow(sheet_df) + 1),
        cols = which(colnames(sheet_df) == "term_name"),
        gridExpand = T
      ) #+1 to account for col headers
      addStyle(wb,
        sheet = sheet_name, style = c_bold,
      ...
      for (i in 1:nrow(vocab_df)) {
        if (any(sheet_df$term_name == vocab_df$term_name[i])) {
          col_start <-
            which(colnames(vocab_df) == "vocab1")
          col_end <-
            which(colnames(vocab_df) == paste0("vocab", vocab_df$n_options[i]))
          col_start_alphabet <-
            LETTERS[col_start] # this is always D
          col_end_alphabet <-
            c(LETTERS, paste0("A", LETTERS))[col_end] # if it exceeds Z, it'll continue with AA, AB, AC etc.
          val <-
            paste0(
              "'Drop-down values'!$",
              col_start_alphabet, "$", i + 1, ":$",
              col_end_alphabet, "$", i + 1
          ...
          dataValidation(wb,
            sheet = sheet_name,
            cols = dataVal_col,
            rows = which(sheet_df$term_name == vocab_df$term_name[i]) + 1, #+1 to acount for the column header
            type = "list",
            value = val,
            showErrorMsg = FALSE,
            allowBlank = TRUE
      ...
      for (i in 1:nrow(vocab_df)) {
        if (any(colnames(sheet_df) == vocab_df$term_name[i])) {
          ...
          dataValidation(wb,
            sheet = sheet_name,
            cols = which(colnames(sheet_df) == vocab_df$term_name[i]),
            rows = term_name_row + 1:100, #+1 because I want the dropdown to start one row below 'term_name'
            type = "list",
            value = val,
            showErrorMsg = FALSE,
            allowBlank = TRUE
