...
eDNA_temp_gen_fun = function(req_lev = c('M', 'R', 'O'), 
  ...
  #MT: Error in loadNamespace(x) : there is no package called ‘tidyr’
  
  
  # create a directory for output templates
  if(dir.exists(paths = "./template")){dir.create(paste('template', project_id, sep='_'))} 
  #MT: the if statemet doesn't creat the template_project_id folder if the tmeplate folder doesn't exist
  ...
  readme1 <- c('The templates were generated using the eDNA checklist version of;', 
               ...
               format(Sys.time(),
                      '%Y-%m-%dT%H:%M:%S'))
#MT :I want the -/+hh:mm time difference  
...
#  iso_current_time <- sub("(\\d{2})(\\d{2})$", "\\1:\\2", iso_current_time)
  ...
  #MT: need to add exp meta user def fields
  ...
  if (detection_type=='multi taxon detection') {
    readme4=c(readme4, # MT: add exp meta for multi taxon. 
              paste('rawOTU', project_id, assay_name, '<library_id>', sep = '_'), #MT: change to otuRaw
              paste('otu', project_id, assay_name, '<library_id>', sep = '_'), #MT: change to otuFinal
              paste('dnaSeqData', project_id, assay_name, '<library_id>', sep = '_'), #Change to taxaRaw, taxaFinal
  ...
  samp_type_row2keep <- if(any(sample_type %>% tolower() == 'other')) {
    ...
    samp_type_row2keep <- apply(input[,sample_type], 1, function(x) any(!is.na(x) & !is.null(x))) ##### MT: IS it chosing na/null rows?
  }
  ...
  for (i in assay_name) {
    saveWorkbook(wb, paste0(Sys.Date(), '_tester_', project_id, '_', i, '_ENTER seq_run_id HERE.xlsx'), overwrite = T)
  }
  #MT: outputs not in template folder?
  #MT: assay specific files are only dnaseqdata, not metadata
  #MT: user defined fields not added?
  #MT: field name orders
  #MT: Section colours
  #MT: M/R/O colours
  #MT: study metadata vertical
  #MT: targeted taxon > only sample metadata is produced
  #MT: missing lots of fields (esp study metadata)
}
...
eDNA_temp_gen_fun(req_lev = c('M', 'R', 'O'), sample_type = c('Water', 'Sediment'), detection_type='targeted taxon detection', project_id = 'gbr2023', assay_name = c('MiFish', 'crust16S')) 
